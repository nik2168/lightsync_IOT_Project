...
export default function SmartHomeScreen() {
  ...
  // Motion alert settings state
  const [motionAlertsEnabled, setMotionAlertsEnabled] = useState(true);
  const [lastAlertTime, setLastAlertTime] = useState<number>(0);

  // ✅ NEW: Button sound state
  const [buttonSound, setButtonSound] = useState<Audio.Sound | null>(null);

  // ✅ NEW: Load button sound effect
  useEffect(() => {
    const loadButtonSound = async () => {
      try {
        ...
        // ✅ MINIMAL: Simple audio configuration that works
  ...
  // ✅ NEW: Function to play button sound effect
  ...
  // ✅ MODIFIED: Create useCallback functions with manual sound trigger
  const handleToggleRedLed = useCallback(
    async (isManualPress = true) => {
      console.log("🔴 Toggling Red LED, current state:", redLedState);

      // ✅ Play sound only on manual press
      if (isManualPress) {
    ...
    [redLedState, dispatch, socket, buttonSound]
  );
  ...
  const handleToggleGreenLed = useCallback(
    ...
    [greenLedState, dispatch, socket, buttonSound]
  );
  ...
  const handleToggleYellowLed = useCallback(
    ...
    [yellowLedState, dispatch, socket, buttonSound]
  );
  ...
  const handleToggleAllLights = useCallback(
    async (isManualPress = true) => {
      ...
      // Toggle individual lights WITHOUT sound (pass false)
      handleToggleYellowLed(false);
      handleToggleGreenLed(false);
      handleToggleRedLed(false);
  ...
  // ✅ NEW: Motion alerts toggle with sound
  const handleToggleMotionAlerts = useCallback(async () => {
    await playButtonSound();
    setMotionAlertsEnabled(!motionAlertsEnabled);
  }, [motionAlertsEnabled, buttonSound]);

  // ✅ NEW: See All button with sound
  const handleSeeAllAlerts = useCallback(async () => {
    await playButtonSound();
    router.push({
      pathname: "/motionalerts",
      params: { alerts: JSON.stringify(motionAlerts) },
    });
  }, [router, motionAlerts, buttonSound]);

  // Advanced motion alert speech function
  const speakMotionAlert = useCallback(
    (alertCount: number, location: string, time: string) => {
      if (!motionAlertsEnabled) {
        console.log("🔇 Motion alerts disabled, skipping speech");
      ...
      const voiceConfig = {
        language: "en-US",
        pitch: alertCount > 5 ? 0.75 : 0.85,
        rate: alertCount > 10 ? 0.7 : 0.8,
        volume: 1.0,
        onDone: () => {
          console.log("✅ Motion alert speech completed");
          setLastAlertTime(currentTime);
  ...
  // Create refs for functions used in event listeners
  const handleToggleRedLedRef = useRef(handleToggleRedLed);
  const handleToggleGreenLedRef = useRef(handleToggleGreenLed);
  const handleToggleYellowLedRef = useRef(handleToggleYellowLed);
  const handleToggleAllLightsRef = useRef(handleToggleAllLights);
  ...
  useEffect(() => {
    ...
    // ✅ Auto-enable yellow LED on motion WITHOUT sound (pass false)
    if (!yellowLedState) {
      console.log("💡 Auto-enabling yellow LED for motion detection");
      handleToggleYellowLedRef.current(false); // false = no sound for automatic trigger
    }
  }, [motionAlerts.length]);
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      >
        ...
        <UpdatePrompt isVisible={isUpdateAvailable} buttonSound={buttonSound} />

        <ScrollView>
          <View className="p-4 py-5 gap-3">
            {/* Temperature Card - No buttons here */}
            <View className="rounded-3xl overflow-hidden">
              <ImageBackground
                source={require("@/assets/home/homeplant.jpg")}
                className="p-4"
            ...
            {/* ✅ ENHANCED: Motion Alert with sound-enabled Toggle */}
            <View className="rounded-2xl">
              <View className="flex-row items-center bg-gray-200 rounded-full px-2 py-2 justify-between">
                <View className="flex-row items-center justify-center">
                  <TouchableOpacity className="w-12 h-12 bg-white flex-row rounded-full items-center justify-center">
                ...
                <View className="flex-row items-center gap-4">
                  <View className="flex-col items-center">
                    <TouchableOpacity
                      onPress={handleToggleMotionAlerts}
                      className={`w-9 h-9 rounded-full items-center justify-center ${
                        motionAlertsEnabled ? "bg-blue-500" : "bg-gray-300"
                      }`}
                    >
            ...
            {/* ✅ ENHANCED: Lights Control with sound */}
            <Text className="text-black text-lg font-semibold px-4 mt-6">
              Home Controls
            </Text>
            <View className="bg-gray-200 rounded-3xl">
              ...
              <View className="flex-row gap-3 p-3 rounded-3xl">
                {/* Lamp 1 */}
                <TouchableOpacity
                  onPress={() => handleToggleRedLed(true)} // true = manual press
                  className="bg-white rounded-2xl flex-row justify-between p-4 flex-1"
                >
                  <View>
                    <Text className="text-black font-semibold mb-1">
                ...
                {/* Lamp 2 */}
                <TouchableOpacity
                  onPress={() => handleToggleYellowLed(true)} // true = manual press
                  className="bg-white rounded-2xl flex-row justify-between p-4 flex-1"
                >
                  <View>
                    <Text className="text-black font-semibold mb-1">
              ...
              <View className="flex-row gap-3 px-3 pb-3 rounded-3xl">
                {/* Fan */}
                <TouchableOpacity
                  onPress={() => handleToggleGreenLed(true)} // true = manual press
                  className="bg-white rounded-2xl flex justify-between p-4 h-[8rem] flex-1"
                >
                  <View>
                    <Text className="text-black font-semibold mb-1">Fan</Text>
                ...
                {/* Everything On */}
                <TouchableOpacity
                  onPress={() => handleToggleAllLights(true)} // true = manual press
                  className="bg-white rounded-2xl flex justify-between p-4 h-[8rem] flex-1"
                >
                  <View>
                    <Text className="text-black font-semibold mb-1">
